...
@implementation SMTEHTMLEditorController

@synthesize textExpander = _textExpander;

// Simple rich-text HTML editor copied from:
// https://developer.mozilla.org/en-US/docs/Rich-Text_Editing_in_Mozilla
//
static NSString* gEditorHtml = @"\
<!doctype html>\
<html>\
<head>\
...
- (void)loadView {
	...
	if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad)
		applicationFrame = CGRectInset(applicationFrame, 10, 10);
	webView = [[UIWebView alloc] initWithFrame: applicationFrame];
	self.view = [webView autorelease];
	
    [webView loadHTMLString: gEditorHtml baseURL: nil];
	if (_textExpander == nil) {
		_textExpander = [[SMTEDelegateController alloc] init];
		_textExpander.fillCompletionScheme = @"tetester-fill-xc";
		_textExpander.fillForAppName = @"TEtouch SDK Tester";
		_textExpander.fillDelegate = self;
	}
	[webView setDelegate: self.textExpander];
}
...
- (void)viewDidLoad
{
	...
	if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
		UIBarButtonItem *button = [[UIBarButtonItem alloc] initWithTitle: @"Done" style: UIBarButtonItemStyleDone target: self action: @selector(done:)];
	...
	if ([uiTextObject isKindOfClass: [NSDictionary class]]) {
		...
		if (self.view == wv) {
			...
			if ((fieldInfo = [uiTextObject objectForKey: SMTEkElementID]) != nil)
				result = [NSString stringWithFormat: @"HTMLview_ID:%@", fieldInfo];
			else if ((fieldInfo = [uiTextObject objectForKey: SMTEkElementName]) != nil)
				result = [NSString stringWithFormat: @"HTMLview_Name:%@", fieldInfo];
		}
	}
...
- (BOOL)prepareForFillSwitch: (NSString*)textIdentifier {
	...
	// to activate.
	// It should save the contents of the editor, but we don't have a way to do that
	return YES;
}
...
{
	NSRange srchRange = [textIdentifier rangeOfString: @"HTMLview_"];
	if (srchRange.location == 0) {
		...
		srchRange = [textIdentifier rangeOfString: @"HTMLview_ID:"];
		if (srchRange.location == 0) {
			return [NSDictionary dictionaryWithObjectsAndKeys: self.view, SMTEkWebView,
					[textIdentifier substringFromIndex: NSMaxRange(srchRange)], SMTEkElementID, nil];
		}
		srchRange = [textIdentifier rangeOfString: @"HTMLview_Name:"];
		if (srchRange.location == 0) {
			return [NSDictionary dictionaryWithObjectsAndKeys: self.view, SMTEkWebView,
